...
useEffect(() => {
  ...
  const getDynamicOptions = (field) => {
    ...
    if (field.options && Array.isArray(field.options) && field.options.length > 0) {
      const activeOptions = field.options.filter(opt => 
        opt && opt.isActive !== false && opt.label && opt.value
      );
  ...
  const validateField = (field, value, allFormData = formData) => {
    ...
    // Pattern validation
    if (validation?.pattern && !new RegExp(validation.pattern).test(value.toString().trim())) {
  ...
  const handleDocumentChange = (e) => {
    ...
    const validFiles = files.filter((file) => {
      if (!allowedTypes.includes(file.type)) {
        alert(`❌ File ${file.name} is not supported. Allowed formats: PDF, DOC, DOCX only`);
        return false;
      }
  ...
  const handleProfilePhotosChange = (e) => {
    ...
    const validFiles = files
      .filter((file) => {
        if (!allowedTypes.includes(file.type)) {
          alert(`❌ File ${file.name} is not supported. Allowed formats: JPG, PNG only`);
          return false;
        }
        if (file.size > maxSize) {
          alert(`❌ File ${file.name} is too large. Maximum size is 100KB`);
    ...
    setHasError((prev) => [...prev, ...new Array(validFiles.length).fill(false)]);
  ...
  const handleFieldBlur = (fieldName) => {
    ...
    if (fieldName === "zipcode" && formData[fieldName] && formData[fieldName].length >= 3) {
      const currentCountry = formData.country || "United States";
  ...
  const getZipCodePattern = (country) => {
    ...
    switch (country) {
      case "India":
        return { pattern: "^[1-9][0-9]{5}$", message: "Enter valid 6-digit Indian PIN code" };
      case "United States":
        return { pattern: "^[0-9]{5}(-[0-9]{4})?$", message: "Enter valid 5-digit US ZIP code" };
      case "United Kingdom":
        return { pattern: "^[A-Z]{1,2}[0-9][A-Z0-9]? ?[0-9][A-Z]{2}$", message: "Enter valid UK postcode" };
      case "Canada":
        return { pattern: "^[A-Z][0-9][A-Z] ?[0-9][A-Z][0-9]$", message: "Enter valid Canadian postal code" };
      case "Australia":
        return { pattern: "^[0-9]{4}$", message: "Enter valid 4-digit Australian postcode" };
      case "Germany":
        return { pattern: "^[0-9]{5}$", message: "Enter valid 5-digit German postcode" };
      default:
        return null;
    }
  };
  ...
  const renderField = (field) => {
    const { name, label, type, placeholder, helpText, isRequired, validation } = field;
    ...
    const inputClassName = `mt-1 block w-full px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 ${
      touched && error ? "border-red-300 focus:border-red-500" : "border-gray-300 focus:border-orange-500"
      ...
      return (
        <div className="relative">
          ...
          <p className="text-xs text-gray-500 mt-1">
            {formData.country ? `Enter ${formData.country} postal code → auto-fills City & State` : "Enter postal code to auto-fill location"}
          </p>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const profileImageBase64 = profilePhotos.length > 0 ? await fileToBase64(profilePhotos[0]) : null;
            ...
            <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
              ...
              <div className="flex justify-end">
                <p className="text-xs sm:text-sm text-gray-600 text-right max-w-xs sm:max-w-none ml-auto">
                  ...
                  <strong className="text-red-600">indicates a required field</strong>
                </p>
              </div>
                    ...
                    {profilePhotos.length < 3 && (
                      ...
                      >
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          <svg className="w-10 h-10 mb-3 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <p className="text-xs text-gray-500">
                            {3 - profilePhotos.length} {profilePhotos.length === 2 ? "slot" : "slots"} remaining • Max 100KB per photo
                          </p>
                        </div>
                      </label>
                    )}
                    ...
                    <div className="mt-4">
                      <div className="grid grid-cols-3 gap-4">
                        {profilePhotos.map((photo, index) => (
                          <div key={index} className="relative group bg-white rounded-lg border-2 border-orange-200 overflow-hidden shadow-sm">
                        ...
                        <div key={index} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                          <div className="flex items-center space-x-3">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-xs text-gray-500">
                                {(doc.size / 1024).toFixed(2)} KB • {getFileType(doc.name)}
                              </p>
                            </div>
                          </div>
                          <button
